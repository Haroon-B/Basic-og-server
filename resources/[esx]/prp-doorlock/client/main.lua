	...
	while true do
		...
		for i=1, #Config.DoorList do
			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, Keys['E']) and not IsPedInAnyVehicle(PlayerPedId(), true) then
                    if isAuthorized then
						doorID.locked = not doorID.locked
                        
						playAnim("missheistfbisetup1", "unlock_enter_janitor", 1500)
                        Citizen.Wait(1500)
						TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 3.0, 'lockdoor', 0.6)
						TriggerServerEvent('esx_doorlock:updateState', i, doorID.locked) -- Broadcast new state of the door to everyone
						ClearPedTasksImmediately(PlayerPedId())
					end
				end
				if IsControlJustReleased(0, Keys['E']) and IsPedInAnyVehicle(PlayerPedId(), true) then
                    if isAuthorized then
                        doorID.locked = not doorID.locked
                        TriggerServerEvent('esx_doorlock:updateState', i, doorID.locked)-- Broadcast new state of the door to everyone
                    end
